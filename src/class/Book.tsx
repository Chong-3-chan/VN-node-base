import { KKVRecord, charaRecord } from '../data/data';
...
export namespace VM {
  ...
  function readProps(VMLines: readonly string[], category: Category, check: boolean) {
    ...
    if (firstLost)
      throw new Error(
        `VM.readProps(): 构造${category}-VM时追加属性不足够或不对应。缺少:${firstLost}\n需要:${propNames.join(',')}。\n读取到:\n${JSON.stringify(
          re,
          null,
          2
  ...
  function getFnsNeedFilekeys(staticSentence: StaticSentence): string[] {
    const map: Record<string, (props: any[]) => string> = {
      setPlace: (props: any[]) => props[0],
      chara: (props: any[]) => charaRecord[props[0]]?.pic[props[1]],
      CG: (props: any[]) => props[0],
      BGM: (props: any[]) => props[0],
      voice: (props: any[]) => props[0],
    };
    return staticSentence.fns
      .map((e) => {
        const [fnName, props] = e;
        const todo = map[fnName];
        return todo !== void 0 ? todo(props) : null;
      })
